...
// First visit starts at the introduction; later visits resume this page's last reading position.
const scrollMemory = (() => {
  if ('scrollRestoration' in history) history.scrollRestoration = 'manual';

  const storageKey = `ways-through:scroll:${location.pathname || '/'}${location.search}`;
  ...
  function readPosition() {
    try {
      ...
      if (!saved || !Number.isFinite(saved.y)) return null;
      return { x: Number.isFinite(saved.x) ? saved.x : 0, y: saved.y };
    } catch {
      return null;
    }
  }
  ...
  function savePosition() {
    ...
    try {
      localStorage.setItem(storageKey, JSON.stringify({ x: Math.round(window.scrollX), y: Math.round(window.scrollY) }));
    } catch {
      // Browsers that block local storage simply retain their normal scroll behaviour.
    }
  }
  ...
  function restorePosition() {
    ...
    // An explicit anchor always wins over a remembered position.
    if (!location.hash) {
      const saved = readPosition();
      const maxY = Math.max(0, document.documentElement.scrollHeight - window.innerHeight);
      const targetY = saved ? Math.min(Math.max(0, saved.y), maxY) : 0;
      window.scrollTo({ left: saved?.x || 0, top: targetY, behavior: 'auto' });
    }

    requestAnimationFrame(() => { readyToSave = true; });
  }
  ...
  if (document.readyState === 'complete') queueRestore();
  else window.addEventListener('load', queueRestore, { once: true });
...
const orbitalSystem = {
  ...
  satellites: [
    { lens: 'frame', orbit: 'outer', phase: 3.83, period: 62, direction: 1 },
    { lens: 'translate', orbit: 'middle', phase: 5.6, period: 48, direction: -1 },
    { lens: 'verify', orbit: 'inner', phase: 2, period: 34, direction: 1 }
  ]
};

function readOrbitTilt() {
  if (!systemMap) return orbitalSystem.tilt;
  const value = getComputedStyle(systemMap).getPropertyValue('--orbit-tilt').trim();
  const degrees = Number.parseFloat(value);
  return Number.isFinite(degrees) ? (degrees * Math.PI) / 180 : orbitalSystem.tilt;
}
...
function positionSystemSatellites(time = 0) {
  ...
  const tilt = orbitalSystem.tilt;
  const cosTilt = Math.cos(tilt);
  const sinTilt = Math.sin(tilt);

  orbitalSystem.satellites.forEach((orbit) => {
    const satellite = systemSatellites.find((item) => item.dataset.lens === orbit.lens);
    const radii = orbitalSystem.orbitRadii[orbit.orbit];
    const orb = satellite?.querySelector('.satellite-orb');
    if (!satellite || !orb || !radii?.x || !radii?.y) return;
    const angularVelocity = (Math.PI * 2) / orbit.period;
    const angle = orbit.phase + time * angularVelocity * orbit.direction;
    ...
    const orbCentreX = orb.offsetLeft + orb.offsetWidth * .5;
    const orbCentreY = orb.offsetTop + orb.offsetHeight * .5;
    satellite.style.transform = `translate3d(${positionX - orbCentreX}px, ${positionY - orbCentreY}px, 0)`;
  });
}
...
function measureSystemMap() {
  ...
  orbitalSystem.tilt = readOrbitTilt();
  orbitalSystem.satellites.forEach((satellite) => {
    const orbit = systemMap.querySelector(`.system-orbit--${satellite.orbit}`);
    if (!orbit) return;
    orbitalSystem.orbitRadii[satellite.orbit] = { x: orbit.offsetWidth * .5, y: orbit.offsetHeight * .5 };
  });
...
const portfolioExplorer = document.querySelector('.portfolio-explorer');
const portfolioTabs = [...document.querySelectorAll('.portfolio-tab')];
const portfolioPanels = [...document.querySelectorAll('[data-portfolio-panel]')];

function setPortfolioChapter(name, focusButton = false) {
  if (!portfolioExplorer || !portfolioTabs.length || !portfolioPanels.length) return;
  const panel = portfolioPanels.find((item) => item.dataset.portfolioPanel === name);
  if (!panel) return;

  portfolioExplorer.dataset.activePortfolio = name;
  selectTab(portfolioTabs, name, 'portfolio', focusButton);
  portfolioPanels.forEach((item) => {
...
bindArrowTabs(portfolioTabs, 'portfolio', setPortfolioChapter);
...
const aiContent = {
  gemini: { label: 'Doubao + Gemini / language, planning, video', title: 'Turn research into a direction people can enter.', text: 'I use Doubao and Gemini to explore copy, shape early project plans, and storyboard short video directions from my own research. I select, rewrite, and set the final voice.', imageSrc: 'assets/ai-gemini-v2.png', imageAlt: 'A dark research installation with translucent layers and a red inquiry line', imageLabel: 'Copy / planning / video' },
  codex: { label: 'Codex + DeepSeek / code and logic', title: 'Turn a chosen direction into a working system.', text: 'I use Codex and DeepSeek to build website interactions, prototypes, and the mathematical logic behind them. I review the behaviour, test edge cases, and refine the result myself.', imageSrc: 'assets/ai-codex-v2.png', imageAlt: 'A graphite and glass system model with finely lit connections', imageLabel: 'Code / logic' },
  hunyuan: { label: 'Hunyuan / modelling and 3D', title: 'Make early form visible.', text: 'I use Hunyuan to explore physical form, material, and spatial mood through 3D model studies. The output stays a starting point for design judgment, not the final answer.', imageSrc: 'assets/ai-hunyuan-v2.png', imageAlt: 'A layered terrain model in clay, resin, and brass', imageLabel: 'Modelling / 3D' }
};
...
const motionItems = document.querySelectorAll('.section-heading, .live-project-list, .case-card, .portfolio-explorer, .personal-explorer, .ai-workflow, .about-grid, .project-hero, .project-notes, .project-end');
